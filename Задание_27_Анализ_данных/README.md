...
В задании 27 нужен именно **медоида** — реальная точка из данных!
...
## 11. Пошаговый алгоритм решения

1. **Прочитать условие**: сколько кластеров N, какая формула расстояния, что именно вывести
...
5. **Проверить**: `print([len(cl) for cl in clusters])` — количество и размеры кластеров должны совпадать с ожидаемым
6. **Найти медоиды** через `get_medoid()`
...
**Когда применять:** кластеры разделяются прямой линией (x < C или y < C).

Визуализируем в LibreOffice Calc: данные → точечная диаграмма → находим границу разреза.
...
**Принцип:** берём точку → находим всех соседей в радиусе eps → добавляем в кластер → для каждого нового соседа снова ищем соседей → повторяем пока кластер растёт.
...
Вызов: `draw_clusters(data, labels)`, где `labels` — список индексов кластера для каждой точки.
