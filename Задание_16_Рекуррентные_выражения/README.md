...
| Ошибка | Как избежать |
|:---|:---|
| Не дойти до базового случая и зациклиться | Всегда проверять, что база определена и достижима |
| Перепутать порядок вычислений (считать сверху вниз, не подставив значения снизу) | Строить таблицу или дерево рекурсии |
| Арифметическая ошибка при ручном подсчёте | Проверять каждый шаг, дважды просматривать подстановку |
| Не учесть, что функция вызывается с теми же аргументами дважды | Запоминать уже вычисленные значения |
| Перепутать базовый случай (например, `n=0` вместо `n=1`) | Внимательно читать условие задания |
